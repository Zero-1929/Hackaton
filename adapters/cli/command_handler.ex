...
defmodule Adapters.CLI.CommandHandler do
  ...
  def handle(comando, usuario) do
    case String.split(comando, " ", parts: 2) do
      ...
      ["/quit"] -> salir()
      ["/participants"] -> listar_participantes()
      [""] -> :ok
  ...
  defp mostrar_ayuda do
    ...
    GESTIÓN DE USUARIOS:
      ...
      /participants           → Lista todos los participantes registrados
  ...
  participantes = ETSRepo.listar_participantes()
  ...
  if Enum.empty?(participantes) do
    ...
    Enum.each(participantes, fn participante ->
      equipo_info = if participante.equipo_id do
        equipo = ETSRepo.obtener_equipo(participante.equipo_id)
        if equipo, do: " (Equipo: #{equipo.nombre})", else: ""
      else
        " (Sin equipo)"
      end

        IO.puts("👤 #{participante.nombre}")
        IO.puts("   ID: #{participante.id}")
        IO.puts("   Email: #{participante.email}")
        IO.puts("   Estado: #{equipo_info}")
        IO.puts(String.duplicate("─", 60))
      end)
